...
class StructureChecker {
  ...
  async mapStructure() {
    ...
    // Check services structure. A feature must ship exactly one variant; when
    // both are present, record 'both' so each is still inspected and the
    // conflict itself is reported.
    const hasFile = this.structure.files.includes('services.ts')
    const hasFolder = this.structure.directories.includes('services')

    if (hasFile && hasFolder) {
      this.structure.services = 'both'
    } else if (hasFile) {
  ...
  async validateDirectories() {
    ...
    const allowedDirs = new Set(['components', 'containers', 'services', 'docs', '__tests__', '__stories__', 'assets'])

    for (const dirName of this.structure.directories) {
      if (!allowedDirs.has(dirName) && !dirName.startsWith('.')) {
        this.warnings.push(`Unexpected directory: ${dirName} (allowed: ${Array.from(allowedDirs).join(', ')})`)
      }
    }
  ...
  async validateComponentsDir() {
    ...
    for (const component of this.structure.components) {
      if (!component.endsWith('-component.tsx')) {
        this.errors.push(`Component file '${component}' should end with '-component.tsx'`)
      }
    }
  }
  ...
  async validateContainersDir() {
    ...
    if (existsSync(storiesDir)) {
      this.errors.push('containers/ should NOT have __stories__/ (only dumb components have stories)')
    }

    // Validate container files
    for (const container of this.structure.containers) {
      if (!container.endsWith('-container.tsx')) {
        this.errors.push(`Container file '${container}' should end with '-container.tsx'`)
      }
    }
  }
  ...
  async validateTestAndStoryDirs() {
    ...
    // A missing __stories__/ is not reported here: `@wl/require-component-stories`
    // (error, from @slip-stream-kit/eslint-plugin) already blocks on a dumb component
    // without a story, using these same defaults. Only the *misplaced* directory is
    // still flagged above, because lint cannot tell a stale components/__stories__/
    // from a component that simply has no story.
  ...
  checkFileNaming() {
    ...
    for (const fileName of this.structure.files) {
      ...
      // Documentation is not code; README.md is the conventional spelling and
      // is not subject to the kebab-case rule for source files.
      ...
      if (fileName[0] === fileName[0].toUpperCase() && fileName[0] !== fileName[0].toLowerCase()) {
        this.errors.push(`File '${fileName}' should use kebab-case, not PascalCase`)
      }

      // Check for underscores (should use hyphens)
      if (fileName.includes('_') && !fileName.startsWith('__')) {
        this.warnings.push(`File '${fileName}' should use hyphens (-) not underscores (_)`)
      }
    }
  ...
  async validateServicesStructure() {
    ...
    if (services === 'file' || services === 'both') {
      await this.validateServicesFile()
    }

    if (services === 'folder' || services === 'both') {
      this.validateServicesFolder()
    }
  ...
  async validateServicesFile() {
    ...
    try {
      ...
      if (lines > 250) {
        this.warnings.push(`services.ts has ${lines} lines (> 250). Consider refactoring to services/ folder`)
      } else {
        this.successes.push(`✓ services.ts size OK (${lines} lines)`)
      }
    } catch (e) {
      this.warnings.push(`Could not read services.ts: ${e.message}`)
    }
  }

  validateServicesFolder() {
    ...
    for (const [fileName, description] of Object.entries(requiredServiceFiles)) {
      ...
      if (!existsSync(filePath)) {
        this.errors.push(`services/ folder missing required file: ${fileName} (${description})`)
      } else {
    ...
    for (const [fileName, description] of Object.entries(recommendedFiles)) {
      const filePath = join(servicesDir, fileName)
      if (!existsSync(filePath)) {
        this.warnings.push(`services/ folder should have ${fileName} (${description})`)
      }
    }
  }
