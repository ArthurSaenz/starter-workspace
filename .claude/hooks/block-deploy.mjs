...
const deny = (reason) => {
  denyDecision(
    [
      ...
      '',
      'No `/infra-kit:*` skills in this session? In a terminal: `infra-kit setup` (updates the plugin) or',
      '`claude plugin update infra-kit@infra-kit --scope project`, then restart Claude Code at the repo root.',
      '',
      'Deploying to a non-prod environment IS allowed — but it goes through infra-kit,',
      'which enforces the deploy rules:',
      '    ik release deploy-all --json --agent          # dev / stage / personal envs',
      '    ik release deploy-selected --json --agent     # a subset of services',
      'Both preview the plan and exit; re-run the same argv with --yes to execute.',
      '',
      "prod is DELIVERED, never deployed ad-hoc — and delivery is a human's call:",
      '    ik release deliver                            # run by a human, in their terminal',
      '',
