...
1. Read the current Turbo version from `devDependencies.turbo` in `package.json` (this is the OLD version, strip the `^` prefix).
...
4. Update `devDependencies.turbo` in `package.json` to `^<new version>`.
